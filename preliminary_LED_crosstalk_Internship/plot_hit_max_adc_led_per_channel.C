...
// root -l -q -b 'plot_hit_max_adc_led_per_channel.C++' -e 'run_macro()'



// To run plot_hit_max_adc_led_per_channel(): root -l -q 'plot_hit_max_adc_led_per_channel.C(run_on,run_off, asic = 0 or 1, port = 0, 1, 2 or 3)'
// example: root -l -q -b 'plot_hit_max_adc_led_per_channel.C(321, 345, 0, 1, "008")'
...
void plot_hit_max_adc_led_per_channel(int run_on = 321, int run_off = 345, int asic = 0, int port = 0, std::string injected_pb = "008")
{
    // gStyle->SetOptStat(0);
...
void run_all_tests() {
    ...
    for (const auto& test : tests) {
        plot_hit_max_adc_led_per_channel(test.run_on, test.run_off, test.asic, test.port, test.pb_id);
    }
}
